...
import React, { useEffect, useRef, useMemo, useState, useImperativeHandle, forwardRef, type ReactNode, type ForwardRefRenderFunction } from 'react';
...
/* 외부에서 "최종 이동결과" 를 넘길 때 사용하는 인터페이스 */
...
const Ecctrl: ForwardRefRenderFunction<React.RefObject<RapierRigidBody>, EcctrlProps> = (
  ...
  const autoBalanceCharacter = () => {
    ...
    if (isModeCameraBased) {
      ...
      slopeRayOriginRef.current.position.x =
        slopeRayOriginOffest * Math.sin(slopeRayOriginUpdatePosition.angleTo(modelFacingVec) * (camBasedMoveCrossVecOnY.y < 0 ? 1 : -1));
      slopeRayOriginRef.current.position.z =
        slopeRayOriginOffest * Math.cos(slopeRayOriginUpdatePosition.angleTo(modelFacingVec) * (camBasedMoveCrossVecOnY.y < 0 ? 1 : -1));
    } else {
  ...
  useFrame((state, delta) => {
    ...
    /* 외부 콜백에서 "최종 이동 결과" 가져오기 */
    if (movementOverride) {
      const { desiredMoveDir, desiredJump, desiredRun } = movementOverride() || {};

      // Move character to the direction provided by the callback
      if (desiredMoveDir && desiredMoveDir.lengthSq() > 0.0001) {
        // Apply direction to model euler
    ...
    // Apply animations (for both input methods)
    if (animated) {
      if (isJumping && canJump) {
        // Jump animation already handled above
