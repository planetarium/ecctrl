...
const Ecctrl = ({
  ...
  const initialGravityScale = React.useMemo(() => {
    var _a;
    return (_a = props.gravityScale) != null ? _a : 1;
  }, []);
  let massRatio = 1;
  ...
  fiber.useFrame((state, delta) => {
    ...
    const { joystickDis, joystickAng, runState, button1Pressed } = joystickValues;
    const keyboardValues = isInsideKeyboardControls ? getKeys() : presetKeys;
    const { forward, backward, leftward, rightward, jump, run } = keyboardValues;
    if (movementOverride) {
      const { desiredMoveDir, desiredJump, desiredRun } = movementOverride() || {};
      if (desiredMoveDir && desiredMoveDir.lengthSq() > 1e-4) {
        modelEuler.y = Math.atan2(desiredMoveDir.x, desiredMoveDir.z);
        moveCharacter(delta, !!desiredRun, slopeAngle, movingObjectVelocity);
        isMoving = true;
        isRunning = !!desiredRun;
      }
      if (desiredJump && canJump) {
        ...
        characterRef.current.setLinvel(
          jumpDirection.set(0, (desiredRun ? sprintJumpMult * jumpVel : jumpVel) * slopJumpMult, 0).projectOnVector(actualSlopeNormalVec).add(jumpVelocityVec),
          true
        );
      ...
      if (controllerIndex !== null) {
        ...
        isMoving = isMoving || gamepadKeys.forward || gamepadKeys.backward || gamepadKeys.leftward || gamepadKeys.rightward;
      }
      if (joystickDis > 0) {
        modelEuler.y = pivot.rotation.y + (joystickAng - Math.PI / 2);
        moveCharacter(delta, runState, slopeAngle, movingObjectVelocity);
        isMoving = true;
        isRunning = isRunning || runState;
      }
      modelEuler.y = ((movingDirection2) => movingDirection2 === null ? modelEuler.y : movingDirection2)(
      ...
      isJumping = (jump || button1Pressed) && canJump;
      if (isJumping) {
        ...
        characterMassForce.y *= jumpForceToGroundMult;
        (_b = rayHit.collider.parent()) == null ? void 0 : _b.applyImpulseAtPoint(characterMassForce, standingForcePoint, true);
        animated && jumpAnimation();
      }
    }
    if (animated) {
      if (isJumping && canJump) ;
      else if (canJump && isMoving) {
        isRunning ? runAnimation() : walkAnimation();
      } else if (!canJump) {
        jumpIdleAnimation();
      } else if (canJump && !isMoving && !isJumping) {
        idleAnimation();
      }
    ...
    if (slopeRayHit) {
      actualSlopeNormal = (_c = slopeRayHit.collider.castRayAndGetNormal(slopeRayCast, slopeRayLength, false)) == null ? void 0 : _c.normal;
  ...
  return /* @__PURE__ */ React.createElement(
    ...
    /* @__PURE__ */ React.createElement(rapier.CapsuleCollider, { name: "character-capsule-collider", args: [capsuleHalfHeight, capsuleRadius] }),
